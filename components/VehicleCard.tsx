...
export function VehicleCard({ vehicle }: { vehicle: VehicleCardData }) {
  ...
  return (
    <div className="group border border-ink/10 bg-white/60 hover:border-stamp/40 transition-colors break-inside-avoid mb-4">
      <Link href={`/vehicles/${vehicle.slug}`} className="block">
        <div className="relative bg-paper-dim overflow-hidden">
          {cover ? (
            <Image
              src={cover}
              alt={vehicle.title}
            ...
            <div className="w-full aspect-[4/3] flex items-center justify-center text-steel-soft font-mono text-xs">
