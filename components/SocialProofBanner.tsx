...
const SOCIAL_STATS = [
  {
    ...
    icon: (
      <svg viewBox="0 0 24 24" fill="none" className="h-9 w-9">
        ...
        <circle cx="12" cy="12" r="4.3" stroke="white" strokeWidth="1.8" />
        <circle cx="17.5" cy="6.5" r="1.15" fill="white" />
      </svg>
    ),
  },
  {
...
export default function SocialProofBanner() {
  return (
    <section className="relative isolate overflow-hidden rounded-2xl">
      ...
      {/* Voile bleu clair pour la lisibilité */}
      ...
      <div className="relative z-10 flex flex-col items-center gap-8 px-6 py-14 text-center sm:px-10 sm:py-16">
        ...
        <div className="max-w-2xl">
          <h2 className="text-2xl font-bold text-white sm:text-3xl">
            Votre partenaire de confiance pour l&apos;export de véhicules
          </h2>
          <p className="mt-2 text-sm text-white/80 sm:text-base">
            Suivez nos dernières arrivées, exports et avis clients en direct de Corée du Sud.
          </p>
        </div>
        ...
        <div className="grid w-full max-w-3xl grid-cols-1 gap-6 sm:grid-cols-3">
          {SOCIAL_STATS.map((s) => (
            <div
              key={s.name}
              className="flex flex-col items-center gap-3 rounded-xl border border-white/25 bg-white/10 px-5 py-6 backdrop-blur-sm"
            >
              <span
                className={`flex h-16 w-16 items-center justify-center rounded-full shadow-lg ${s.bg}`}
              >
                {s.icon}
              </span>
              ...
              {s.sub && (
                <span className="-mt-2 text-xs uppercase tracking-wide text-white/70">
                  {s.sub}
                </span>
              )}
              ...
              <Link
                ...
                className="mt-1 w-full rounded-lg bg-[#0F3D8C] px-4 py-2.5 text-sm font-semibold text-white transition hover:bg-[#0B2E68]"
              >
                {s.cta}
              </Link>
            </div>
